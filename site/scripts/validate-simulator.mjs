...
assert(demoStart.includes('demo-cli.mjs') && demoStart.includes('open "$VIEWER_URL"'), 'demo_start가 Shell CLI 실행 후 Viewer를 자동으로 열어야 합니다.');
assert(demoStart.includes('reset=1') && page.includes("params.get('reset') === '1'"), '새 발표 시작 시 이전 Web 실행 상태를 초기화해야 합니다.');
assert(demoCli.includes("import { buildStages } from '../lib/scenario.ts'"), 'Shell CLI는 scenario.ts의 단일 Build Data를 사용해야 합니다.');
assert(demoCli.includes('for (const stage of buildStages)') && demoCli.includes('input.question'), 'Shell CLI 단일 입력 후 STEP 1~5 자동 실행이 누락됐습니다.');

assert(page.includes('<MermaidChart') && page.includes("type Screen = 'viewer' | 'product'"), 'Web은 최종 Mermaid Viewer부터 시작해야 합니다.');
assert(!page.includes('buildState') && !page.includes('prompt-form'), '브라우저에 Harness Build 가짜 CLI가 남아 있습니다.');
...
console.log('- Shell fake CLI: 1 input → 5 × 60-second stages');
